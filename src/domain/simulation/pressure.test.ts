...
describe('applyPressure — increasing-drought', () => {
  ...
  it('reduces moisture by 1 on even eras', () => {
    const world = makeWorld('increasing-drought', { temperature: 5, moisture: 5, fertility: 5, shelter: 5 });
  ...
  it('clamps moisture at 0', () => {
    const world = makeWorld('increasing-drought', { temperature: 5, moisture: 0, fertility: 5, shelter: 5 });
    const { regions } = applyPressure(world, 2);
    expect(regions[0]!.conditions.moisture).toBe(0);
    expect(regions[0]!.conditions.fertility).toBe(4); // 0 <= 3, so reduced
  });
...
describe('applyPressure — cooling-climate', () => {
  ...
  it('reduces temperature by 1 on even eras', () => {
    const world = makeWorld('cooling-climate', { temperature: 5, moisture: 5, fertility: 5, shelter: 5 });
    const { regions } = applyPressure(world, 2);
  ...
  it('clamps temperature at 0', () => {
    const world = makeWorld('cooling-climate', { temperature: 0, moisture: 5, fertility: 5, shelter: 5 });
    const { regions } = applyPressure(world, 2);
    expect(regions[0]!.conditions.temperature).toBe(0);
  });
});
